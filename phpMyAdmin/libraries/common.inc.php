...
if (! isset($cfg['Servers']) || count($cfg['Servers']) == 0) {
    ...
    $conn_error = "<b>MariaDB 5</b> or <b>MariaDB 10</b> is stopped or has not been installed. Please check the package on your DSM first.";
} else {
    ...
    if (! empty($cfg['Server'])) {
        ...
        if (! $cfg['Server']['AllowNoPassword']
			...
			if($cfg['Server']['user'] == 'root') {
				$login_user_is_root = true;
				$login_server = $cfg['Server']['verbose'];
			}
