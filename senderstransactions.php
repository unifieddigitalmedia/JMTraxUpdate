...
<table class=" table-bordered" style='font-size:80%;text-align:center;color:black;position:relative; margin:0px auto;width:100%;'>
    ...
    <tbody>
      <tr ng-repeat='daily in dailytransactionlist' style="cursor:pointer;" ng-click="getstatement($index)">
        ...
        <td class="tablecell">{{daily.recipientfirstname}}</td>
        <td class="tablecell">{{daily.recipientsurname}}</td>
...
$scope.sendersname = getCookie('sender_firstname') + "  " + getCookie('sender_lastname') ;


$http.get("api/sender_transactions.php?username="+getCookie('sender_username')+"&usertype=customer" ).then(function(response) {
...
document.cookie = "receipient_firstname=" + $scope.dailytransactionlist[para].recipientfirstname;
document.cookie = "receipient_lastname=" + $scope.dailytransactionlist[para].recipientsurname;


window.location = "sender_receipient_transactions.php" ;
